...
def reply_callback(channel, method, properties, body): # required signature for a callback; no return
    ...
    for row in rows:
        if not 'correlation_id' in row:
            print('Warning for corrids.csv: no "correlation_id" for a payment:', row)
            continue
...
if __name__ == "__main__":
    print("This is " + os.path.basename(__file__) + ": listening for a reply from payment for a transaction...")
    receiveOrder()
